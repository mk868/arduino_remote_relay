...
static void send_body_module(Print &res, module_t *module) {
    ...
    res.print(module->id);
    res.print(F("\", \"name\": \""));
    store_module_get_name(module->id, res);
    res.print(F("\",\"value\": "));
...
static bool handle_modules(const server_request_t &req, Print &res) {
    ...
    for (int i = 0; module_all[i]; i++) {
        ...
        if (!str_prefix(req.path + root_path_len + path_separator_len, module->id)) {
            continue;
        }
        auto module_id_len = strlen(module->id);
        if (req.path[root_path_len + path_separator_len + module_id_len] != 0 &&
            req.path[root_path_len + path_separator_len + module_id_len] != '?') {
            continue;
        }

        const char *name_query = "?name=";
        const uint8_t name_query_len = strlen(name_query);

        if (str_prefix(req.path + root_path_len + path_separator_len + module_id_len, name_query)) {
            const char *value = req.path + root_path_len + path_separator_len + module_id_len + name_query_len;
            Serial.print("new name: ");
            Serial.println(value);
            store_module_set_name(module->id, value);
        }

        const char *value_query = "?value=";
        const uint8_t value_query_len = strlen(value_query);

        if (str_prefix(req.path + root_path_len + path_separator_len + module_id_len, value_query)) {
            char value = req.path[root_path_len + path_separator_len + module_id_len + value_query_len];
        ...
        if (str_prefix(req.path + root_path_len + path_separator_len + module_id_len, initial_value_query)) {
            char value = req.path[root_path_len + path_separator_len + module_id_len +
            ...
            if (prev_value != new_value) {
                ...
                store_module_set_init_value(module->id, new_value);
            }
        }
